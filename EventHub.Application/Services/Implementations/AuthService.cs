...
namespace EventHub.Application.Services.Implementations
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<AuthResponseDto> RegisterUserAsync(UserRegisterDto dto)
        {
            ...
            {
                // **COMPENSATION: delete the uploaded photo (no DB rollback here - no tx)
            ...
            {
                // **COMPENSATION: delete the uploaded photo
                await DeleteUploadedFilesAsync(profilePhotoUrl, null, null);
                return new AuthResponseDto
                {
                    IsAuthenticated = false,
        ...
        public async Task<AuthResponseDto> RefreshTokenAsync(RefreshTokenDto payload)
        {
            ...
            // Step 1: Look up the refresh token in the DB FIRST — the DB is the source of truth,
            ...
            // Step 3: Validate the JWT WITHOUT the lifetime check — an expired access token
            ...
            // Step 5: The access token must actually be expired — that's the only reason to refresh.
            ...
            // Step 8: Rotation — consume the old token so it can never be replayed.
